    ...
    D: DelayMs<u8> + DelayUs<u16>,
{
    pub fn new(i2c: I2C, mut delay: D, addr: u8) -> Result<Self, Error> {
        let drv =
            HD44780::new_i2c_mcp23008(i2c, addr, true, &mut delay).map_err(|_| Error::InitMcp)?;
    ...
    pub fn write_msg(&mut self, /* id: u8, */ msg: String) -> Result<u8, Error> {
        ...
        self.drv
            .set_cursor_pos(0, &mut self.delay)
        ...
        for (i, c) in msg.chars().enumerate() {
            if i % 20 == 0 && i != 0 {
                self.drv
                    .set_cursor_pos(i as u8, &mut self.delay)
                    .map_err(|_| Error::SetCursorPos)?;
            }

            if c.is_ascii() {
                self.drv
                    .write_byte(c as u8, &mut self.delay)
                    .map_err(|_| Error::WriteStr)?;
            }
        }
